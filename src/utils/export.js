...
export const downloadCSV = (data, events = [], appName = 'adidas_vibes') => {
    ...
    const headers = ["Code,Result,City,Region,Timestamp,Event,EventStartDate,EventEndDate,AgeRange,Gender,Redeemed,RedeemedAt,Q1,Q2,Q3,Q4,Q5,Q6"];
    ...
    const rows = data.map(row => {
        ...
        return [
            ...
            row.redeemed ? "YES" : "NO",
            row.redeemedAt ? row.redeemedAt.toDate?.() : "",
            clean(q.q1), clean(q.q2), clean(q.q3), clean(q.q4), clean(q.q5), clean(q.q6)
        ].join(",");
    })?.sort?.((a, b) => {
        return (new Date(b?.split?.(',')?.[4]).getTime()) - (new Date(a?.split?.(',')?.[4]).getTime());
    }) || []; // Sort by timestamp
